...
for (const entry of readdirSync(SRC)) {
  try {
    ...
    if (entry === "_redirects") {
      console.warn(
        `[finalize] skip ${entry}: ${e.code || e.message} (pre-injected, identical content)`,
      );
    } else {
      console.error(
        `[finalize] FAILED copying ${entry} into dist/: ${e.code || e.message} — aborting (a partial dist/ deploys broken)`,
      );
      process.exit(1);
    }
  }
